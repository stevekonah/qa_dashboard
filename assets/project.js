...
const state = { data: [], project: "", filtered: [] };
...
async function init() {
  ...
  if (target) { sel.value = target; state.project = target; render(); }
...
function render() {
  state.filtered = state.data.filter(d => d.project === state.project).sort((a,b) => (a.today||"").localeCompare(b.today||""));
  renderHeader(); renderStats(); renderPillarChart(); renderStageChart();
  renderPerformanceLog(); renderConcerns(); renderResponses();
}
...
function renderPillarChart() {
  ...
  new Chart(document.getElementById("pillarChart"), {
    type:"bar",
    data:{ labels:["Programmatic","MEL"], datasets:[{ label:"Avg score (%)", data:[avg(prog), avg(mel)], backgroundColor:["#1E88E5","#7B1FA2"] }] },
...
function renderStageChart() {
  ...
  new Chart(document.getElementById("stageChart"), {
    type:"bar",
    data:{ labels: stages, datasets:[{ label:"Avg score (%)", data: values, backgroundColor:["#3C6B52","#294B39","#1F3D2E"] }] },
    options:{ scales:{ y:{ max:100 } }, plugins:{ legend:{ display:false } }, maintainAspectRatio:false },
  });
}
...
function renderResponses() {
  ...
  document.querySelector("#responsesTable tbody").innerHTML = filtered.length
    ? filtered.map(r => {
        ...
        return `<tr><td>${r.question}</td><td>${r.answer ?? "—"}</td><td><span class="badge ${cls}">${r.score ?? "—"}</span></td><td>${r.concern ? '<span class="badge red">Concern</span>' : ""}</td></tr>`;
      }).join("")
